...
async function fetchData(url) {
    try {
        ...
        if (!response.ok) {
            //Improved error handling.  Checks for HTTP errors
...
async function fetchHoldersFromHelius(mintAddress) {
    ...
    const body = {
        jsonrpc: '2.0',
        id: 'r5-check',
        method: 'getTokenLargestAccounts',  //CHECK THIS METHOD NAME
        params: [mintAddress]
    };
    try {
        ...
        if (!res.ok) {
            throw new Error(`Helius HTTP error! status: ${res.status}`);
        }
...
async function fetchSolscanFallback(mint, lpTokenAddr) {
    ...
    let renounced = null;
    let lpLocked = null;
    let error = null; // Added error variable

    try {
        const tokenMeta = await fetch(`${baseURL}/token/meta?tokenAddress=${mint}`, { headers });
        if (!tokenMeta.ok) {
             throw new Error(`Solscan HTTP error! status: ${tokenMeta.status}`);
        }
        ...
        if (lpTokenAddr) {
            const holdersRes = await fetch(`${baseURL}/token/holders?tokenAddress=${lpTokenAddr}&limit=10`, { headers });
             if (!holdersRes.ok) {
                  throw new Error(`Solscan HTTP error! status: ${holdersRes.status}`);
             }
    ...
    return { renounced, lpLocked, error }; // Return the error
}
...
async function getRaydiumPool(mintAddress) {
    ...
    const url = `https://api.helius.xyz/v0/account-info?account=${poolAddress}&api-key=${process.env.HELIUS_KEY}`;
     try {
        const response = await fetch(url);
        if (!response.ok) {
             throw new Error(`Helius HTTP error! status: ${response.status}`);
        }
        ...
        const parsedData = accountInfo.result?.data?.parsed;  //Added question marks
        ...
        return parsedData;
         ...
         return {error: "Error fetching raydium data" + error.message}
     }
}
...
app.post('/api/scan', async (req, res) => {
    ...
    try {
        ...
        const solscanResult = await fetchSolscanFallback(base.address, pair.lpToken?.address);
        if (liquidityLock.locked === null) liquidityLock.locked = solscanResult.lpLocked;
        if (liquidityLock.renounced === null) liquidityLock.renounced = solscanResult.renounced;
        const solscanError = solscanResult.error;
        ...
        if(solscanError){
             responseData.solscanError = solscanError;
        }
